...
export default function Matches(){
 ...
 return <main className="archivePage"><header className="archiveNav"><Link className="brand" href="/">PARS<span>DATABASE</span></Link><Link href="/">← Home</Link></header><section className="archiveHero"><p className="eyebrow">FROM 1912 TO TODAY</p><h1>Matches</h1><p>Results, opponents and competitions from the live historical database.</p></section><section className="archiveContent"><div className="archiveToolbar"><h2>Match archive</h2><span>{loading?'Loading live records…':`${matches.length} matches imported`}</span></div><div className="dataList">{matches.map(m=><Link className="matchRow" href={`/matches/${m.id}/`} key={m.id}><span>{m.played_on} · {m.competition_season?.competition?.name||'Match'}</span><strong>{m.home?.name} <em>{m.home_score ?? '–'} - {m.away_score ?? '–'}</em> {m.away?.name}</strong><b>→</b></Link>)}</div>{!loading&&matches.length===0&&<p className="migrationNote">No match records were returned by the live database.</p>}</section></main>;
}
